...
/*
 * The flow command callback lives in librte_ethdev
 * (rte_flow_parser_cmd_flow_cb). The SET command handler is
 * testpmd-owned -- it uses the library only for pattern/action item
 * tokenization via rte_flow_parser_set_item_tok().
 */
...
/* Forward declaration needed for config init. */
...
/* ---- Testpmd-owned SET raw/sample/ipv6_ext command handler ---- */
...
/* Token for subcommand */
static const char *testpmd_set_subcmds[] = {
			...
			void *res, unsigned int ressize)
{
	...
	/* Initialize the output buffer for item parsing. */
	...
	/*
	 * Initialize library context for item tokenization.
...
/* Token populator for the full SET command */
static void
...
/* Dispatch callback when SET command is complete */
static void
