...
The library provides string-to-structure parsing via three simple helpers:

``rte_flow_parser_parse_attr_str()``
   Parse attribute strings such as ``ingress priority 5`` into
   ``struct rte_flow_attr``.

``rte_flow_parser_parse_pattern_str()``
   Parse pattern strings such as ``eth / ipv4 src is 10.0.0.1 / end``
   into an array of ``struct rte_flow_item``.

``rte_flow_parser_parse_actions_str()``
   Parse action strings such as ``queue index 3 / end`` into an array of
