...
/** Internal parser state used by rte_flow parser helpers. */
struct rte_flow_parser {
	uint32_t max_pattern_items; /**< Maximum supported pattern length. */
	uint32_t max_actions; /**< Maximum supported action list length. */
	int socket_id; /**< NUMA socket used for allocations. */
};

/** Token array produced by the parser lexer. */
struct token_list {
	char **data; /**< Token storage. */
	size_t count; /**< Number of valid tokens. */
	size_t capacity; /**< Allocated token slots. */
};

/**
 * Populate parser defaults when no explicit attributes are provided.
 *
 * @param parser
 *   Parser instance being initialised.
 * @param attr
 *   Optional configuration supplied by the caller.
 */
static void
parser_defaults(struct rte_flow_parser *parser,
	      const struct rte_flow_parse_attr *attr)
{
...
/**
 * Release tokens owned by the tokenizer.
 *
 * @param list
 *   Token array to reset.
 */
static void
...
/**
 * Append a token to the token array, growing storage if required.
 *
 * @param parser
 *   Parser context providing allocation socket information.
 * @param list
 *   Token list to update.
 * @param start
 *   Pointer to first character of the token.
 * @param len
...
/**
 * Check whether a character terminates tokens.
 *
 * @param c
 *   Character to inspect.
 *
 * @return
 *   true when the character is a delimiter, false otherwise.
 */
static bool
...
/**
 * Tokenize a command string into the token list.
 *
 * @param parser
 *   Parser context providing allocation information.
 * @param input
 *   Null-terminated command string.
 * @param out
 *   Result token list to populate.
 *
 * @return
	 ...
	 struct token_list *out)
{
	...
	while (*p) {
		...
		while (*p && !is_token_delim(*p))
			++p;
		if (start != p) {
			int ret = token_list_append(parser, out, start,
					(size_t)(p - start));
			if (ret)
				return ret;
		}
...
/**
 * Populate a flow error structure.
 *
 * @param error
 *   Flow error structure provided by caller.
 * @param code
 *   Error code to report.
 * @param type
 *   Flow error type describing the failure domain.
 * @param msg
 *   Human readable description.
 *
 * @return
 *   Negative error code suitable for return statements.
 */
static int
error_set(struct rte_flow_error *error, int code,
	  enum rte_flow_error_type type,
	  const char *msg)
{
 ...
 *
 * @param token
 *   Null-terminated token string.
 * @param value
 *   Output destination for the parsed integer.
 *
 * @return
 *   0 on success, -EINVAL on invalid syntax.
 */
static int
...
/**
 * Parse an IPv4 address token.
 *
 * @param token
 *   Token representing an IPv4 address.
 * @param addr
 *   Output buffer receiving the address in network order.
 *
 * @return
 *   0 on success, -EINVAL on invalid syntax.
 */
...
/**
 * Parse an IPv6 address token.
 *
 * @param token
 *   Token representing an IPv6 address.
 * @param dst
 *   Output array receiving the address bytes in network order.
 *
 * @return
 *   0 on success, -EINVAL on invalid syntax.
 */
...
/**
 * Validate that the number of entries fits within the configured limits.
 *
 * @param limit
 ...
 * @param error
 *   Error structure to populate on overflow.
 * @param what
 *   Description of the capacity being exceeded.
 *
 * @return
 *   0 when space remains, negative error otherwise.
 */
static int
ensure_capacity(uint32_t limit, uint32_t count,
	       struct rte_flow_error *error, const char *what)
{
	if (count >= limit)
		return error_set(error, ENOMEM,
			RTE_FLOW_ERROR_TYPE_UNSPECIFIED, what);
	return 0;
}

/**
 * Append an ETH pattern item to the flow pattern array.
 *
 * @param parser
 *   Parser context (unused).
 * @param item
 *   Pattern slot to populate.
 * @param error
 *   Error structure (unused since operation cannot fail).
 *
 * @return
 *   Always 0.
 */
static int
parse_item_eth(struct rte_flow_parser *parser __rte_unused,
...
/**
 * Parse an IPv4 pattern item from tokens.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 *   Token sequence describing the item.
 * @param idx
 *   Current index within the token list (advanced on success).
 * @param item
 *   Pattern entry to populate.
 * @param error
 *   Error structure receiving detailed information.
 *
 * @return
...
/**
 * Parse an IPv6 pattern item from tokens.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 *   Token sequence describing the item.
 * @param idx
 *   Current token index (advanced on success).
 * @param item
 *   Pattern entry to populate.
 * @param error
 *   Error structure receiving detailed information.
	        ...
	        struct rte_flow_error *error)
{
	...
	while (*idx < tokens->count) {
		...
		if (!strcasecmp(token, "src") || !strcasecmp(token, "dst")) {
			...
			if (!strcasecmp(qualifier, "spec") ||
			    !strcasecmp(qualifier, "is")) {
				++(*idx);
				if (*idx >= tokens->count)
					goto ipv6_syntax_error;
				char *value = tokens->data[*idx];
				...
				if (parse_ipv6_addr(value, is_src ?
					...
					goto ipv6_syntax_error;
				if (is_src)
					memset(mask->hdr.src_addr.a, 0xFF,
					       sizeof(mask->hdr.src_addr.a));
				else
					memset(mask->hdr.dst_addr.a, 0xFF,
					       sizeof(mask->hdr.dst_addr.a));
				++(*idx);
				continue;
			}
...
/**
 * Free memory allocated for pattern items.
 *
 * @param items
 *   Pattern array to clean up.
 * @param count
...
/**
 * Free memory allocated for action entries.
 *
 * @param actions
 *   Action array to clean up.
 * @param count
 *   Number of valid entries in the array.
 */
static void
...
/**
 * Parse the pattern section of a flow command.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 ...
 *   Current token index (advanced on success).
 * @param result
 *   Output object receiving pattern information.
 * @param error
 *   Flow error to populate on failure.
 *
 * @return
 *   0 on success, negative value otherwise.
 */
static int
parse_pattern_section(struct rte_flow_parser *parser,
	       ...
	       struct rte_flow_error *error)
{
	...
	while (*idx < tokens->count) {
		...
		int cap = ensure_capacity(parser->max_pattern_items, count,
					      error,
		...
		if (!strcasecmp(token, "eth")) {
			...
			if (parse_item_ipv4(parser, tokens, idx, &items[count],
					   error)) {
				pattern_cleanup(items, count);
				rte_free(items);
				return -1;
			...
			if (parse_item_ipv6(parser, tokens, idx, &items[count],
					   error)) {
				pattern_cleanup(items, count);
				rte_free(items);
				return -1;
...
/**
 * Populate a DROP action.
 *
 * @param action
 *   Action slot to populate.
 *
 * @return
 *   Always 0.
 */
...
/**
 * Parse a MARK action from tokens.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 ...
 *   Current token index (advanced on success).
 * @param action
 *   Action entry to populate.
 * @param error
 *   Flow error receiving details on failure.
 *
 * @return
 *   0 on success, negative value otherwise.
 */
...
/**
 * Parse a QUEUE action from tokens.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
...
/**
 * Parse a JUMP action from tokens.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
...
/**
 * Parse the actions section of a flow command.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 ...
 *   Current token index (advanced on success).
 * @param result
 *   Output object receiving action information.
 * @param error
 *   Flow error to populate on failure.
 *
 * @return
 ...
 */
static int
parse_actions_section(struct rte_flow_parser *parser,
	       struct token_list *tokens,
	       size_t *idx, struct rte_flow_parse_result *result,
	       struct rte_flow_error *error)
{
	...
	while (*idx < tokens->count) {
		...
		if (!strcasecmp(token, "drop")) {
			...
			if (parse_action_mark(parser, tokens, idx, &actions[count],
					   error)) {
				actions_cleanup(actions, count);
				rte_free(actions);
				return -1;
			...
			if (parse_action_queue(parser, tokens, idx, &actions[count],
					    error)) {
				actions_cleanup(actions, count);
				rte_free(actions);
				return -1;
			...
			if (parse_action_jump(parser, tokens, idx, &actions[count],
					   error)) {
				actions_cleanup(actions, count);
				rte_free(actions);
				return -1;
...
/**
 * Initialise a parse result helper structure.
 *
 * @param result
		    ...
		    struct rte_flow_error *error)
{
	...
	if (!parser) {
		if (error)
			return (struct rte_flow_parser *)(uintptr_t)
...
int
rte_flow_parser_reset(struct rte_flow_parser *parser __rte_unused,
	       struct rte_flow_error *error __rte_unused)
{
	return 0;
}
...
/**
 * Parse a flow rule command into rte_flow structures.
 *
 * @param parser
 *   Parser context used for allocations.
 * @param tokens
 *   Token sequence representing the parsed rule.
 * @param result
 *   Output object receiving parsed attributes, pattern, and actions.
 * @param error
 *   Flow error receiving details on failure.
 *
 * @return
...
/**
 * Parse tokens representing only a pattern array.
 */
static int
...
/**
 * Parse tokens representing only an actions array.
 */
static int
	 ...
	 struct rte_flow_error *error)
{
	...
	*pattern_out = NULL;
	...
	ret = parse_pattern_tokens(parser, &tokens, pattern_out,
			pattern_count, error);
	token_list_reset(&tokens);
	 ...
	 struct rte_flow_error *error)
{
	...
	*actions_out = NULL;
	...
	ret = parse_actions_tokens(parser, &tokens, actions_out,
			actions_count, error);
	token_list_reset(&tokens);
