...
/* parser_command_* functions removed in v14 (ops infrastructure removed) */
...
{
	uint16_t idx = in->port;

	switch (in->command) {
	case RTE_FLOW_PARSER_CMD_SET_SAMPLE_ACTIONS:
		parser_ctx_set_sample_actions(idx, in->args.vc.actions,
					      in->args.vc.actions_n);
		break;
	case RTE_FLOW_PARSER_CMD_SET_IPV6_EXT_PUSH:
		parser_ctx_set_ipv6_ext_push(idx, in->args.vc.pattern,
					     in->args.vc.pattern_n);
		break;
	case RTE_FLOW_PARSER_CMD_SET_IPV6_EXT_REMOVE:
		parser_ctx_set_ipv6_ext_remove(idx, in->args.vc.pattern,
					       in->args.vc.pattern_n);
		break;
	case RTE_FLOW_PARSER_CMD_SET_RAW_ENCAP:
		parser_ctx_set_raw_common(true, idx, in->args.vc.pattern,
					  in->args.vc.pattern_n);
		break;
	case RTE_FLOW_PARSER_CMD_SET_RAW_DECAP:
		parser_ctx_set_raw_common(false, idx, in->args.vc.pattern,
					  in->args.vc.pattern_n);
		break;
...
RTE_EXPORT_EXPERIMENTAL_SYMBOL(rte_flow_parser_cmd_flow_tok, 26.07);
RTE_EXPORT_EXPERIMENTAL_SYMBOL(rte_flow_parser_cmd_set_raw_tok, 26.07);
