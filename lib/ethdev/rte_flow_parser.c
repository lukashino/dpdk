...
/* RSS_HASH_KEY_LENGTH now defined in rte_flow_parser_cmdline.h */
...
parser_token_to_command(enum parser_token token)
{
	...
	default:
		RTE_LOG(ERR, ETHDEV, "unknown parser token %u\n",
			(unsigned int)token);
	       ...
	       unsigned int size)
{
	 ...
	 * time (see rte_flow_parser_cmd_flow_cb / cmd_set_raw_cb).
	 */
